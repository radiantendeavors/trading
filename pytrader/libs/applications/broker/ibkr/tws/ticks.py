...
class BrokerTicks(ticks.BasicTicks):
    ...
    def request_ticks(self, contract: contract.Contract):
        """!
        Requests ticks from TWS
        """
        req_id = self.brokerclient.req_tick_by_tick_data(self.queue, contract)
    ...
    def _send_ticks(self, data):
        """!
        Send tick data to the strategies.
        """
        req_id = data["req_id"]
        ticker = self.data_tickers[req_id]
        tick = data["tick"]
        message = {"tick": {ticker: tick}}
        self.data_queue.put(message)

...
class StockInfo(Base):
    ...
    def __repr__(self):
        return "<StockInfo(ticker='%s', last_seen='%s')" % (self.ticker,
                                                            self.last_seen)

    def add_stock_info(self, *args, **kwargs):
    ...
    def get_ticker_list(self, *args, **kwargs):
        ticker_list = []
        ticker_object = DBSession.query(StockInfo).filter_by(
...
class StockDaily(Base):
    ...
    def add_stock_daily(self, *args, **kwargs):
        ...
        print(daily_info)
        daily_info.to_sql("stock_daily", engine, if_exists="append")
